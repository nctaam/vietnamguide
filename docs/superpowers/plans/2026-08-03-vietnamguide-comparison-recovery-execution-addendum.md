...
The repository confirms that these required files are missing:
...
Mechanically restore those exact files from surviving Codex JSONL patch history stored on the safe C volume. Pair each patch call with its successful result, replay only target-specific successful patches in chronological order, and never reconstruct file contents from memory. Record the JSONL call IDs or exact line numbers, expected filenames, final raw byte sizes, and SHA-256 values.
...
php -l .\ops\verify-guide-experience-live.php
