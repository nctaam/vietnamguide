# -*- coding: utf-8 -*-
"""
VietnamGuide Stage 62: Apply Semantic Internal Linking Mesh for 7 New Pillars
"""
...
def main():
    print("=== Applying Stage 62 Link Mesh to Production VPS ===")
    
    with open('ops/stage62_mesh_ops.json', 'r', encoding='utf-8') as f:
        operations = json.load(f)

    print(f"Loaded {len(operations)} operations from ops/stage62_mesh_ops.json")
    ...
    sftp = ssh.open_sftp()
    with sftp.file('/tmp/stage62_mesh_ops.json', 'w') as f:
...
$json_str = file_get_contents('/tmp/stage62_mesh_ops.json');
...
"""
    ...
    with sftp.file('/tmp/apply_stage62_mesh.php', 'w') as f:
        f.write(php_script)
    sftp.close()

    print("Executing mesh update via WP-CLI on production VPS...")
    cmd = f"wp eval-file /tmp/apply_stage62_mesh.php --path={WP_PATH} --allow-root"
    ...
    cmd_purge = f"wp litespeed-purge all --path={WP_PATH} --allow-root"
    stdin, stdout, stderr = ssh.exec_command(cmd_purge)
    print(stdout.read().decode('utf-8'))

    ssh.close()
    print("Stage 62 Link Mesh execution complete.")
