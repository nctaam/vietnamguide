...
sftp = ssh.open_sftp()
with sftp.file('/tmp/get_links61.php', 'w') as f:
    f.write(php_code)
sftp.close()

stdin, stdout, stderr = ssh.exec_command('wp eval-file /tmp/get_links61.php --path=/usr/local/lsws/vietnamguide.net/html --allow-root')
posts_data = json.loads(stdout.read().decode('utf-8'))
ssh.exec_command('rm -f /tmp/get_links61.php')
ssh.close()
...
for t in targets:
    sources = [p['post_name'] for p in posts_data if t in (p['post_content'] or '') and p['post_name'] != t]
