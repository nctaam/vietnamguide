# -*- coding: utf-8 -*-
"""
VietnamGuide Stage 65: Apply Semantic Internal Linking Mesh for 7 New Pillars
"""
...
def main():
    print("=== Applying Stage 65 Link Mesh to Production VPS ===")
    
    with open('ops/stage65_mesh_ops.json', 'r', encoding='utf-8') as f:
        operations = json.load(f)

    print(f"Loaded {len(operations)} operations from ops/stage65_mesh_ops.json")
    ...
    sftp = ssh.open_sftp()
    with sftp.file('/tmp/stage65_mesh_ops.json', 'w') as f:
...
$json_str = file_get_contents('/tmp/stage65_mesh_ops.json');
...
foreach ($ops as $idx => $op) {
    ...
    $wpdb->update(
        $wpdb->posts,
        [
            'post_modified'     => '2026-09-22 10:00:00',
            'post_modified_gmt' => '2026-09-22 10:00:00'
        ],
        ['ID' => $postId]
    );
...
"""
    ...
    with sftp.file('/tmp/apply_stage65_mesh.php', 'w') as f:
        f.write(php_script)
    sftp.close()

    print("Executing mesh update via WP-CLI on production VPS...")
    cmd = f"wp eval-file /tmp/apply_stage65_mesh.php --path={WP_PATH} --allow-root"
    ...
    ssh.exec_command("rm -f /tmp/apply_stage65_mesh.php /tmp/stage65_mesh_ops.json")
    ssh.close()
