# -*- coding: utf-8 -*-
"""
VietnamGuide Stage 72: Deploy 7 High-Intent Pillars to Production VPS
"""
...
import content_where_to_stay_mai_chau
import content_where_to_stay_pu_luong
import content_where_to_stay_con_dao
import content_vietnam_family_travel_guide
import content_central_vietnam_itinerary
import content_hcmc_mekong_budget
import content_hanoi_to_hue_transport
...
MODULES = [
    content_where_to_stay_mai_chau,
    content_where_to_stay_pu_luong,
    content_where_to_stay_con_dao,
    content_vietnam_family_travel_guide,
    content_central_vietnam_itinerary,
    content_hcmc_mekong_budget,
    content_hanoi_to_hue_transport,
]
...
def main():
    print(f"=== Deploying {len(POSTS)} Stage 72 Pillars to VPS ===")
    ...
    # SFTP JSON data
    sftp = ssh.open_sftp()
    with sftp.file('/tmp/stage72_posts.json', 'w') as f:
...
$posts = json_decode(file_get_contents('/tmp/stage72_posts.json'), true);
$inserted = [];
...
foreach ($posts as $p) {
    ...
    if ($post_id && !is_wp_error($post_id)) {
        ...
        $wpdb->update(
            $wpdb->posts,
            [
                'post_modified' => '2026-09-23 12:45:00',
                'post_modified_gmt' => '2026-09-23 12:45:00'
            ],
            ['ID' => $post_id]
        );
        $inserted[] = [
...
file_put_contents('/tmp/stage72_deployed.json', json_encode($inserted));
"""
    with sftp.file('/tmp/deploy_stage72.php', 'w') as f:
        f.write(php_script)
    sftp.close()

    print("Executing PHP insertion via WP-CLI...")
    cmd = f"wp eval-file /tmp/deploy_stage72.php --path={WP_PATH} --allow-root"
    ...
    sftp = ssh.open_sftp()
    with sftp.file('/tmp/stage72_deployed.json', 'r') as f:
        deployed_data = json.load(f)
    sftp.close()

    # Clean up temp files
    ssh.exec_command("rm -f /tmp/deploy_stage72.php /tmp/stage72_posts.json /tmp/stage72_deployed.json")
    ...
    if all_ok:
        print("\nALL 7 STAGE 72 PILLARS LIVE & VERIFIED ON PRODUCTION VPS!")
    else:
