# -*- coding: utf-8 -*-
"""
VietnamGuide Stage 77: Deploy 7 High-Intent Pillars to Production VPS
Expands coverage from 220 to 227 pages.
"""
...
import content_where_to_stay_dong_hoi
import content_da_lat_to_mui_ne_transport
import content_where_to_stay_pleiku
import content_vietnam_scooter_rental_checklist
import content_hanoi_to_mu_cang_chai_transport
import content_where_to_stay_mu_cang_chai
import content_da_nang_to_phong_nha_transport
...
MODULES = [
    content_where_to_stay_dong_hoi,
    content_da_lat_to_mui_ne_transport,
    content_where_to_stay_pleiku,
    content_vietnam_scooter_rental_checklist,
    content_hanoi_to_mu_cang_chai_transport,
    content_where_to_stay_mu_cang_chai,
    content_da_nang_to_phong_nha_transport,
]
...
def main():
    print(f"=== Deploying {len(POSTS)} Stage 77 Pillars to VPS (220 -> 227 pages) ===")
    ...
    sftp = ssh.open_sftp()
    with sftp.file('/tmp/stage77_posts.json', 'w') as f:
...
$posts = json_decode(file_get_contents('/tmp/stage77_posts.json'), true);
$inserted = [];
    ...
    if ($post_id && !is_wp_error($post_id)) {
        ...
        $wpdb->update(
            ...
            [
                ...
                'post_modified_gmt' => '2026-09-23 16:30:00'
            ],
            ['ID' => $post_id]
        );
        $inserted[] = [
...
file_put_contents('/tmp/stage77_deployed.json', json_encode($inserted));
"""
    with sftp.file('/tmp/deploy_stage77.php', 'w') as f:
        f.write(php_script)
    sftp.close()

    print("Executing PHP insertion via WP-CLI...")
    cmd = f"wp eval-file /tmp/deploy_stage77.php --path={WP_PATH} --allow-root"
    ...
    sftp = ssh.open_sftp()
    with sftp.file('/tmp/stage77_deployed.json', 'r') as f:
        deployed_data = json.load(f)
    sftp.close()

    ssh.exec_command("rm -f /tmp/deploy_stage77.php /tmp/stage77_posts.json /tmp/stage77_deployed.json")
    ...
    if all_ok:
        print("\nALL 7 STAGE 77 PILLARS LIVE & VERIFIED ON PRODUCTION VPS!")
    else:
