...
$applyPath = Join-Path $repoRoot 'ops/apply-vietnam-food-safety-street-food-etiquette-post.php'
$verifyPath = Join-Path $repoRoot 'ops/verify-vietnam-food-safety-street-food-etiquette-post.php'
...
Require-File 'Vietnam food safety static verifier' $staticPath
Require-File 'Vietnam food safety post apply script' $applyPath
Require-File 'Vietnam food safety post runtime verifier' $verifyPath
...
Require-Contains 'Apply script targets expected slug' $apply 'vietnam-food-safety-street-food-etiquette'
...
Require-Contains 'Apply script checks target meta' $apply 'vg_food_safety_post_assert_target_meta'
Require-Contains 'Apply script checks exact target date before overwrite' $apply "'vg_editorial_target_publish_date' => '2026-08-08'"
...
foreach ($needle in @(
    'Vietnam.travel - Beginner''s guide to Vietnamese street food',
    ...
    'eat street food, but choose context well',
    'source-diversity'
)) {
...
Require-Contains 'Runtime verifier checks exact target date' $verify '2026-08-08'
...
Write-Output 'Vietnam food safety and street food etiquette post static verification passed.'
