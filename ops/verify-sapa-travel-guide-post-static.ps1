...
$applyPath = Join-Path $repoRoot 'ops/apply-sapa-travel-guide-post.php'
$verifyPath = Join-Path $repoRoot 'ops/verify-sapa-travel-guide-post.php'
...
Require-File 'Sapa Travel Guide static verifier' $staticPath
Require-File 'Sapa Travel Guide apply script' $applyPath
Require-File 'Sapa Travel Guide runtime verifier' $verifyPath
...
Require-Contains 'Apply script targets expected slug' $apply 'sapa-travel-guide'
Require-Contains 'Apply script guards exact post ID' $apply '$post_id !== 518'
Require-Contains 'Runtime verifier guards exact post ID' $verify '$post_id !== 518'
Require-Contains 'Runtime verifier guards exact title' $verify "post_title !== 'Sapa Travel Guide: Terraces, Trekking and Softer Mountain Travel'"
Require-Contains 'Runtime verifier guards exact slug' $verify "post_name !== 'sapa-travel-guide'"
...
Require-Contains 'Apply script checks target date before overwrite' $apply "'vg_editorial_target_publish_date' => '2026-08-23'"
...
Require-Contains 'Apply script prevalidates categories' $apply '$category_term_ids = vg_sapa_travel_guide_post_term_ids'
Require-Contains 'Apply script prevalidates tags' $apply '$tag_term_ids = vg_sapa_travel_guide_post_term_ids'
...
foreach ($needle in @(
    ...
    'Vietnam.travel - Sapa for sustainable travellers',
    'Vietnam.travel - My Sapa: Ly Thy Ker',
    'Vietnam.travel - Why Fansipan is a must-do in Sapa',
...
Require-Contains 'Runtime verifier enforces zero raw external body links' $verify 'Sapa Travel Guide raw_external_body_href_count should be 0; found {$raw_external_body_href_count}'
Require-Contains 'Runtime verifier enforces zero rendered external body links' $verify 'Sapa Travel Guide rendered_external_body_href_count should be 0; found {$rendered_external_body_href_count}'
Require-Contains 'Runtime verifier enforces zero external body links' $verify 'Sapa Travel Guide external_body_href_count should be 0; found {$external_body_href_count}'
Require-Contains 'Runtime verifier extracts internal hrefs' $verify 'vg_verify_sapa_travel_guide_internal_body_hrefs'
...
Require-Contains 'Runtime verifier checks rendered internal href targets' $verify 'vg_verify_sapa_travel_guide_assert_internal_body_hrefs_published($failures, $rendered_content)'
Require-Contains 'Runtime verifier checks content depth' $verify '12000'
Require-Contains 'Runtime verifier checks target publish date' $verify '2026-08-23'
...
Write-Output 'Sapa Travel Guide static checks passed.'
