# -*- coding: utf-8 -*-
"""
VietnamGuide Stage 63: Deploy 7 High-Intent Pillars to Production VPS
"""
...
sys.path.append('ops')
import content_where_to_stay_danang
import content_where_to_stay_hoian
import content_halong_day_vs_overnight
import content_hanoi_to_halong
import content_danang_to_hue
import content_vietnam_april
import content_tap_water
...
MODULES = [
    content_where_to_stay_danang,
    content_where_to_stay_hoian,
    content_halong_day_vs_overnight,
    content_hanoi_to_halong,
    content_danang_to_hue,
    content_vietnam_april,
    content_tap_water,
]
...
def main():
    print("=== Deploying Stage 63 Pillars (7 Posts) to Production VPS ===")
    ...
    sftp = ssh.open_sftp()
    with sftp.file('/tmp/stage63_pillars.json', 'w') as f:
...
$json_str = file_get_contents('/tmp/stage63_pillars.json');
...
"""
    ...
    with sftp.file('/tmp/deploy_stage63.php', 'w') as f:
        f.write(php_script)
    sftp.close()

    print("Executing deployment via WP eval-file...")
    cmd = f"wp eval-file /tmp/deploy_stage63.php --path={WP_PATH} --allow-root"
    ...
    ssh.exec_command("rm -f /tmp/deploy_stage63.php /tmp/stage63_pillars.json")
