...
$applyPath = Join-Path $repoRoot 'ops/apply-best-vietnam-routes-first-time-visitors-post.php'
$verifyPath = Join-Path $repoRoot 'ops/verify-best-vietnam-routes-first-time-visitors-post.php'
...
Require-File 'Best Vietnam routes static verifier' $staticPath
Require-File 'Best Vietnam routes post apply script' $applyPath
Require-File 'Best Vietnam routes post runtime verifier' $verifyPath
...
Require-Contains 'Apply script targets expected slug' $apply 'best-vietnam-routes-first-time-visitors'
...
Require-Contains 'Apply script checks target meta' $apply 'vg_best_routes_post_assert_target_meta'
Require-Contains 'Apply script checks exact target date before overwrite' $apply "'vg_editorial_target_publish_date' => '2026-08-02'"
...
foreach ($needle in @(
    ...
    'Vietnam.travel - Traveller''s guide to Vietnam''s airports',
    'Vietnam.travel - Northern Vietnam',
    'Vietnam.travel - Central Vietnam',
    'Vietnam.travel - Southern Vietnam',
    'Vietnam.travel - Ha Long destination page',
    'Wikimedia Commons image direct URL',
    'open-jaw',
    'maximum-coverage itinerary',
    'source-diversity'
)) {
...
Require-Contains 'Runtime verifier checks content depth' $verify '10500'
Require-Contains 'Runtime verifier checks target publish date' $verify 'vg_editorial_target_publish_date'
Require-Contains 'Runtime verifier checks exact target date' $verify '2026-08-02'
...
Write-Output 'Best Vietnam routes first-time visitors post static verification passed.'
