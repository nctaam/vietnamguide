# -*- coding: utf-8 -*-
"""
VietnamGuide Stage 73: Deploy 7 High-Intent Pillars to Production VPS
"""
...
import content_where_to_stay_cao_bang
import content_where_to_stay_phu_yen
import content_where_to_stay_ba_be
import content_pu_luong_trekking_routes
import content_southern_vietnam_itinerary
import content_vietnam_vegetarian_travel
import content_hanoi_to_cao_bang_transport
...
MODULES = [
    content_where_to_stay_cao_bang,
    content_where_to_stay_phu_yen,
    content_where_to_stay_ba_be,
    content_pu_luong_trekking_routes,
    content_southern_vietnam_itinerary,
    content_vietnam_vegetarian_travel,
    content_hanoi_to_cao_bang_transport,
]
...
def main():
    print(f"=== Deploying {len(POSTS)} Stage 73 Pillars to VPS ===")
    ...
    # SFTP JSON data
    sftp = ssh.open_sftp()
    with sftp.file('/tmp/stage73_posts.json', 'w') as f:
...
$posts = json_decode(file_get_contents('/tmp/stage73_posts.json'), true);
$inserted = [];
    ...
    if ($post_id && !is_wp_error($post_id)) {
        ...
        $wpdb->update(
            ...
            [
                ...
                'post_modified_gmt' => '2026-09-23 14:35:00'
            ],
            ['ID' => $post_id]
        );
        $inserted[] = [
...
file_put_contents('/tmp/stage73_deployed.json', json_encode($inserted));
"""
    with sftp.file('/tmp/deploy_stage73.php', 'w') as f:
        f.write(php_script)
    sftp.close()

    print("Executing PHP insertion via WP-CLI...")
    cmd = f"wp eval-file /tmp/deploy_stage73.php --path={WP_PATH} --allow-root"
    ...
    sftp = ssh.open_sftp()
    with sftp.file('/tmp/stage73_deployed.json', 'r') as f:
        deployed_data = json.load(f)
    sftp.close()

    # Clean up temp files
    ssh.exec_command("rm -f /tmp/deploy_stage73.php /tmp/stage73_posts.json /tmp/stage73_deployed.json")
    ...
    if all_ok:
        print("\nALL 7 STAGE 73 PILLARS LIVE & VERIFIED ON PRODUCTION VPS!")
    else:
