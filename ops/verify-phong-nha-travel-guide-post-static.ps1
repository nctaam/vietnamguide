...
$applyPath = Join-Path $repoRoot 'ops/apply-phong-nha-travel-guide-post.php'
$verifyPath = Join-Path $repoRoot 'ops/verify-phong-nha-travel-guide-post.php'
...
Require-File 'Phong Nha static verifier' $staticPath
Require-File 'Phong Nha apply script' $applyPath
Require-File 'Phong Nha runtime verifier' $verifyPath
...
Require-Contains 'Apply script targets expected slug' $apply 'phong-nha-travel-guide'
Require-Contains 'Apply script guards exact post ID' $apply '$post_id !== 503'
Require-Contains 'Runtime verifier guards exact post ID' $verify '$post_id !== 503'
Require-Contains 'Runtime verifier guards exact title' $verify "post_title !== 'Phong Nha Travel Guide: Caves, Seasons and Route Fit'"
Require-Contains 'Runtime verifier guards exact slug' $verify "post_name !== 'phong-nha-travel-guide'"
...
Require-Contains 'Apply script checks target date before overwrite' $apply "'vg_editorial_target_publish_date' => '2026-08-21'"
...
Require-Contains 'Apply script prevalidates categories' $apply '$category_term_ids = vg_phong_nha_post_term_ids'
Require-Contains 'Apply script prevalidates tags' $apply '$tag_term_ids = vg_phong_nha_post_term_ids'
...
foreach ($needle in @(
    'UNESCO World Heritage Centre - Phong Nha-Ke Bang National Park and Hin Nam No National Park',
    'UNESCO World Heritage Centre - Decision 47 COM 8B.6',
    'Vietnam.travel - Phong Nha',
    'Phong Nha-Ke Bang National Park Management Board',
    'Official park page - Phong Nha Cave',
    'Official park page - Dai A, Hang Over, Hang Pygmy route',
    'Oxalis Adventure - How to choose the right cave tour',
    'Oxalis Adventure - Son Doong Expedition',
    'Oxalis Adventure - Tu Lan Expedition',
    'Oxalis Adventure - Booking Conditions',
    'Airports Corporation of Vietnam - Dong Hoi Airport',
    'Vietnam Railways official booking',
...
Require-Contains 'Runtime verifier enforces zero raw external body links' $verify 'Phong Nha raw_external_body_href_count should be 0; found {$raw_external_body_href_count}'
Require-Contains 'Runtime verifier enforces zero rendered external body links' $verify 'Phong Nha rendered_external_body_href_count should be 0; found {$rendered_external_body_href_count}'
Require-Contains 'Runtime verifier enforces zero external body links' $verify 'Phong Nha external_body_href_count should be 0; found {$external_body_href_count}'
Require-Contains 'Runtime verifier extracts internal hrefs' $verify 'vg_verify_phong_nha_internal_body_hrefs'
...
Require-Contains 'Runtime verifier checks rendered internal href targets' $verify 'vg_verify_phong_nha_assert_internal_body_hrefs_published($failures, $rendered_content)'
Require-Contains 'Runtime verifier checks content depth' $verify '11000'
Require-Contains 'Runtime verifier checks target publish date' $verify '2026-08-21'
...
Write-Output 'Phong Nha static checks passed.'
