# -*- coding: utf-8 -*-
"""
VietnamGuide Stage 81: Deploy 7 High-Intent Pillars to Production VPS
Expands coverage from 248 to 255 pages.
"""
...
def main():
    print(f"=== Deploying {len(POSTS)} Stage 81 Pillars to VPS (248 -> 255 pages) ===")
    ...
    sftp = ssh.open_sftp()
    with sftp.file('/tmp/stage81_posts.json', 'w') as f:
...
$posts = json_decode(file_get_contents('/tmp/stage81_posts.json'), true);
$inserted = [];
    ...
    if ($post_id && !is_wp_error($post_id)) {
        ...
        $wpdb->update(
            ...
            [
                ...
                'post_modified_gmt' => '2026-09-25 14:00:00'
            ],
            ['ID' => $post_id]
        );
        $inserted[] = [
...
file_put_contents('/tmp/stage81_deployed.json', json_encode($inserted));
"""
    with sftp.file('/tmp/deploy_stage81.php', 'w') as f:
        f.write(php_script)
    sftp.close()

    print("Executing PHP insertion via WP-CLI...")
    cmd = f"wp eval-file /tmp/deploy_stage81.php --path={WP_PATH} --allow-root"
    ...
    sftp = ssh.open_sftp()
    with sftp.file('/tmp/stage81_deployed.json', 'r') as f:
        deployed_data = json.load(f)
    sftp.close()

    ssh.exec_command("rm -f /tmp/deploy_stage81.php /tmp/stage81_posts.json /tmp/stage81_deployed.json")
    ...
    if all_ok:
        print("\nALL 7 STAGE 81 PILLARS LIVE & VERIFIED ON PRODUCTION VPS!")
    else:
