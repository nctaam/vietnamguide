...
$applyPath = Join-Path $repoRoot 'ops/apply-vietnam-in-february-post.php'
$verifyPath = Join-Path $repoRoot 'ops/verify-vietnam-in-february-post.php'
...
Require-File 'Vietnam in February static verifier' $staticPath
Require-File 'Vietnam in February post apply script' $applyPath
Require-File 'Vietnam in February post runtime verifier' $verifyPath
...
Require-Contains 'Apply script targets expected slug' $apply 'vietnam-in-february'
...
Require-Contains 'Apply script checks exact target date before overwrite' $apply "'vg_editorial_target_publish_date' => '2026-08-13'"
Require-Contains 'Apply script checks batch before overwrite' $apply "'vg_editorial_batch' => 'batch-2-evergreen-planning'"
Require-Contains 'Apply script prevalidates categories' $apply '$category_term_ids = vg_february_post_term_ids'
Require-Contains 'Apply script prevalidates tags' $apply '$tag_term_ids = vg_february_post_term_ids'
...
foreach ($needle in @(
    ...
    'February route decision',
    'Tet timing',
    'post-Tet restart',
    'central and southern route value',
    'north warming slowly'
)) {
...
Require-Contains 'Runtime verifier checks target publish date' $verify '2026-08-13'
...
Write-Output 'Vietnam in February static checks passed.'
