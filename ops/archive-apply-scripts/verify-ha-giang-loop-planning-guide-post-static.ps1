...
$applyPath = Join-Path $repoRoot 'ops/apply-ha-giang-loop-planning-guide-post.php'
$verifyPath = Join-Path $repoRoot 'ops/verify-ha-giang-loop-planning-guide-post.php'
...
Require-File 'Ha Giang Loop static verifier' $staticPath
Require-File 'Ha Giang Loop apply script' $applyPath
Require-File 'Ha Giang Loop runtime verifier' $verifyPath
...
Require-Contains 'Apply script targets expected slug' $apply 'ha-giang-loop-planning-guide'
Require-Contains 'Apply script guards exact post ID' $apply '$post_id !== 519'
Require-Contains 'Runtime verifier guards exact post ID' $verify '$post_id !== 519'
Require-Contains 'Runtime verifier guards exact title' $verify "post_title !== 'Ha Giang Loop Planning Guide: Safety, Scenery and Route Fit'"
Require-Contains 'Runtime verifier guards exact slug' $verify "post_name !== 'ha-giang-loop-planning-guide'"
...
Require-Contains 'Apply script checks target date before overwrite' $apply "'vg_editorial_target_publish_date' => '2026-08-24'"
...
Require-Contains 'Apply script prevalidates categories' $apply '$category_term_ids = vg_ha_giang_loop_post_term_ids'
Require-Contains 'Apply script prevalidates tags' $apply '$tag_term_ids = vg_ha_giang_loop_post_term_ids'
...
foreach ($needle in @(
    ...
    'UNESCO - Dong Van Karst Plateau UNESCO Global Geopark',
    'Dong Van Karst Plateau Geopark official site',
    'Global Geoparks Network map',
...
Require-Contains 'Runtime verifier reports missing related routes shortcode output' $verify 'Ha Giang Loop rendered output is missing related routes shortcode title id.'
...
Require-Contains 'Runtime verifier reports missing rendered block output' $verify 'Ha Giang Loop rendered output is missing {$label}: {$needle}'
...
Require-Contains 'Runtime verifier enforces zero raw external body links' $verify 'Ha Giang Loop raw_external_body_href_count should be 0; found {$raw_external_body_href_count}'
Require-Contains 'Runtime verifier enforces zero rendered external body links' $verify 'Ha Giang Loop rendered_external_body_href_count should be 0; found {$rendered_external_body_href_count}'
Require-Contains 'Runtime verifier enforces zero external body links' $verify 'Ha Giang Loop external_body_href_count should be 0; found {$external_body_href_count}'
Require-Contains 'Runtime verifier extracts internal hrefs' $verify 'vg_verify_ha_giang_loop_internal_body_hrefs'
...
Require-Contains 'Runtime verifier checks rendered internal href targets' $verify 'vg_verify_ha_giang_loop_assert_internal_body_hrefs_published($failures, $rendered_content)'
Require-Contains 'Runtime verifier checks content depth' $verify '12000'
Require-Contains 'Runtime verifier checks target publish date' $verify '2026-08-24'
...
Write-Output 'Ha Giang Loop static checks passed.'
