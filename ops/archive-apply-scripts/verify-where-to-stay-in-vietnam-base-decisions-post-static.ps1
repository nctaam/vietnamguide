...
$applyPath = Join-Path $repoRoot 'ops/apply-where-to-stay-in-vietnam-base-decisions-post.php'
$verifyPath = Join-Path $repoRoot 'ops/verify-where-to-stay-in-vietnam-base-decisions-post.php'
...
Require-File 'Where to Stay in Vietnam static verifier' $staticPath
Require-File 'Where to Stay in Vietnam post apply script' $applyPath
Require-File 'Where to Stay in Vietnam post runtime verifier' $verifyPath
...
Require-Contains 'Apply script targets expected slug' $apply 'where-to-stay-in-vietnam-base-decisions'
...
Require-Contains 'Apply script checks target meta' $apply 'vg_stay_vietnam_post_assert_target_meta'
Require-Contains 'Apply script checks exact target date before overwrite' $apply "'vg_editorial_target_publish_date' => '2026-08-09'"
...
foreach ($needle in @(
    ...
    'choose the city base before choosing the hotel',
    'source-diversity'
)) {
...
Require-Contains 'Runtime verifier checks exact target date' $verify '2026-08-09'
...
Write-Output 'Where to Stay in Vietnam base decisions post static verification passed.'
