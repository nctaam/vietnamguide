# -*- coding: utf-8 -*-
"""
VietnamGuide Stage 67: Deploy 7 High-Intent Pillars to Production VPS
"""
...
import content_vietnam_august
import content_where_to_stay_hue
import content_mekong_floating_markets
import content_hanoi_to_phong_nha
import content_da_lat_coffee_farms
import content_hoi_an_tailoring
import content_vietnam_september
...
MODULES = [
    content_vietnam_august,
    content_where_to_stay_hue,
    content_mekong_floating_markets,
    content_hanoi_to_phong_nha,
    content_da_lat_coffee_farms,
    content_hoi_an_tailoring,
    content_vietnam_september,
]
...
def main():
    print("=== Deploying Stage 67 Pillars (7 Posts) to Production VPS ===")
    ...
    sftp = ssh.open_sftp()
    with sftp.file('/tmp/stage67_pillars.json', 'w') as f:
...
$json_str = file_get_contents('/tmp/stage67_pillars.json');
...
"""
    ...
    with sftp.file('/tmp/deploy_stage67.php', 'w') as f:
        f.write(php_script)
    sftp.close()

    print("Executing deployment via WP eval-file...")
    cmd = f"wp eval-file /tmp/deploy_stage67.php --path={WP_PATH} --allow-root"
    ...
    ssh.exec_command("rm -f /tmp/deploy_stage67.php /tmp/stage67_pillars.json")
    ssh.exec_command(f"wp litespeed-purge all --path={WP_PATH} --allow-root")
    ssh.close()

    print("Verifying live HTTP 200 responses...")
    urls = [
        "https://vietnamguide.net/plan/vietnam-in-august/",
        ...
        "https://vietnamguide.net/destinations/hoi-an-tailoring-guide/",
