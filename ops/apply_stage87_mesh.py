# -*- coding: utf-8 -*-
"""
VietnamGuide Stage 87: Apply Internal Link Mesh to Production VPS
"""
...
def main():
    ...
    ops_file = os.path.join(ops_dir, 'stage87_mesh_updates.json')
    ...
    print(f"=== Applying Stage 87 Mesh: {len(mesh_dict)} posts to update ===")
    ...
    sftp = ssh.open_sftp()
    with sftp.file('/tmp/stage87_mesh_apply.json', 'w') as f:
...
$mesh = json_decode(file_get_contents('/tmp/stage87_mesh_apply.json'), true);
$count = 0;
    ...
    if (is_wp_error($result)) {
        ...
        $wpdb->update(
            ...
            [
                ...
                'post_modified_gmt' => '2026-09-26 11:05:00'
            ],
            ['ID' => $post_id]
        );
...
"""
    ...
    with sftp.file('/tmp/apply_stage87_mesh.php', 'w') as f:
        f.write(php_script)
    sftp.close()

    print("Executing mesh update script via WP-CLI...")
    cmd = f"wp eval-file /tmp/apply_stage87_mesh.php --path={WP_PATH} --allow-root"
    ...
    ssh.exec_command("rm -f /tmp/stage87_mesh_apply.json /tmp/apply_stage87_mesh.php")
    ssh.close()
    print("Stage 87 mesh application completed!")
