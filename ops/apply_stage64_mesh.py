# -*- coding: utf-8 -*-
"""
VietnamGuide Stage 64: Apply Semantic Internal Linking Mesh for 7 New Pillars
"""
...
def main():
    print("=== Applying Stage 64 Link Mesh to Production VPS ===")
    
    with open('ops/stage64_mesh_ops.json', 'r', encoding='utf-8') as f:
        operations = json.load(f)

    print(f"Loaded {len(operations)} operations from ops/stage64_mesh_ops.json")
    ...
    sftp = ssh.open_sftp()
    with sftp.file('/tmp/stage64_mesh_ops.json', 'w') as f:
...
$json_str = file_get_contents('/tmp/stage64_mesh_ops.json');
...
"""
    ...
    with sftp.file('/tmp/apply_stage64_mesh.php', 'w') as f:
        f.write(php_script)
    sftp.close()

    print("Executing mesh update via WP-CLI on production VPS...")
    cmd = f"wp eval-file /tmp/apply_stage64_mesh.php --path={WP_PATH} --allow-root"
    ...
    ssh.exec_command("rm -f /tmp/apply_stage64_mesh.php /tmp/stage64_mesh_ops.json")
    ssh.close()
