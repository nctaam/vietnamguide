# -*- coding: utf-8 -*-
"""
VietnamGuide Stage 78: Fetch Mesh Target Posts
Exports the post_content of target posts that will provide inbound links to Stage 78 pillars.
"""
...
def main():
    print(f"=== Fetching {len(TARGET_SLUGS)} Stage 78 Mesh Targets from VPS ===")
    ...
    sftp = ssh.open_sftp()
    with sftp.file('/tmp/stage78_slugs.json', 'w') as f:
        f.write(json.dumps(TARGET_SLUGS))

    php_script = """<?php
$slugs = json_decode(file_get_contents('/tmp/stage78_slugs.json'), true);
$results = [];
...
file_put_contents('/tmp/stage78_targets.json', json_encode($results, JSON_UNESCAPED_UNICODE));
"""
    with sftp.file('/tmp/fetch_stage78.php', 'w') as f:
        f.write(php_script)
    sftp.close()

    print("Running WP-CLI target export script...")
    cmd = f"wp eval-file /tmp/fetch_stage78.php --path={WP_PATH} --allow-root"
    ...
    sftp = ssh.open_sftp()
    local_path = os.path.join(ops_dir, 'stage78_mesh_targets.json')
    sftp.get('/tmp/stage78_targets.json', local_path)
    sftp.close()

    ssh.exec_command("rm -f /tmp/stage78_slugs.json /tmp/fetch_stage78.php /tmp/stage78_targets.json")
    ssh.close()
