# -*- coding: utf-8 -*-
"""
VietnamGuide Stage 67: Apply Internal Link Mesh to Production VPS
"""
...
def main():
    ...
    mesh_file = os.path.join(ops_dir, 'stage67_mesh_ops.json')
    ...
    print(f"=== Applying Stage 67 Mesh: {len(mesh_dict)} posts to update ===")
    ...
    sftp = ssh.open_sftp()
    with sftp.file('/tmp/stage67_mesh_apply.json', 'w') as f:
...
$mesh = json_decode(file_get_contents('/tmp/stage67_mesh_apply.json'), true);
$count = 0;
...
"""
    with sftp.file('/tmp/apply_stage67_mesh.php', 'w') as f:
        f.write(php_script)
    sftp.close()

    cmd = f"wp eval-file /tmp/apply_stage67_mesh.php --path={WP_PATH} --allow-root"
    ...
    ssh.exec_command("rm -f /tmp/apply_stage67_mesh.php /tmp/stage67_mesh_apply.json")
