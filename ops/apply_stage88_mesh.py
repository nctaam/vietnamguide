# -*- coding: utf-8 -*-
"""
VietnamGuide Stage 88: Apply Internal Link Mesh to Production VPS
"""
...
def main():
    ...
    ops_file = os.path.join(ops_dir, 'stage88_mesh_updates.json')
    ...
    print(f"=== Applying Stage 88 Mesh: {len(mesh_dict)} posts to update ===")
    ...
    sftp = ssh.open_sftp()
    with sftp.file('/tmp/stage88_mesh_apply.json', 'w') as f:
...
$mesh = json_decode(file_get_contents('/tmp/stage88_mesh_apply.json'), true);
$count = 0;
    ...
    if (is_wp_error($result)) {
        ...
        $wpdb->update(
            ...
            [
                ...
                'post_modified_gmt' => '2026-09-26 12:30:00'
            ],
            ['ID' => $post_id]
        );
...
"""
    ...
    with sftp.file('/tmp/apply_stage88_mesh.php', 'w') as f:
        f.write(php_script)
    sftp.close()

    print("Executing mesh update script via WP-CLI...")
    cmd = f"wp eval-file /tmp/apply_stage88_mesh.php --path={WP_PATH} --allow-root"
    ...
    ssh.close()
    print("Stage 88 mesh deployment completed successfully.")
