# -*- coding: utf-8 -*-
"""
VietnamGuide Stage 59: Route Budget Link Mesh Applier
Inserts 12 high-value contextual editorial links pointing to the 3 route budget hubs:
- /costs/ha-giang-loop-cost-budget/
- /costs/da-nang-hoi-an-budget/
- /costs/hanoi-ninh-binh-ha-long-budget/
"""
import paramiko
import json
...
def main():
    print("=== Applying Stage 59 Route Budget Link Mesh Insertions ===")
    with open('ops/route_budget_mesh_ops.json', 'r', encoding='utf-8') as f:
        ops = json.load(f)
    ...
    sftp = ssh.open_sftp()
    with sftp.file('/tmp/stage59_mesh_ops.json', 'w') as f:
...
$json_str = file_get_contents('/tmp/stage59_mesh_ops.json');
...
"""
    ...
    with sftp.file('/tmp/apply_stage59_mesh_links.php', 'w') as f:
        f.write(php_code)
    sftp.close()

    print("Executing remote Stage 59 link mesh insertion script...")
    cmd = f"wp eval-file /tmp/apply_stage59_mesh_links.php --path={WP_PATH} --allow-root"
    ...
    ssh.exec_command("rm -f /tmp/apply_stage59_mesh_links.php /tmp/stage59_mesh_ops.json")
    ...
    ssh.close()
    print("=== Stage 59 Route Budget Link Mesh Insertion Complete ===")
