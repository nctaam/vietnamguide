# -*- coding: utf-8 -*-
"""
VietnamGuide Stage 84: Deploy 7 High-Intent Pillars to Production VPS
Expands coverage from 269 to 276 pages.
"""
...
import content_where_to_stay_in_dong_van
import content_where_to_stay_in_cao_bang
import content_ha_long_to_ninh_binh_transport
import content_dong_hoi_to_hue_transport
import content_da_lat_to_mui_ne_transport
import content_where_to_stay_in_vinh_hy
import content_quy_nhon_to_nha_trang_transport
...
MODULES = [
    content_where_to_stay_in_dong_van,
    content_where_to_stay_in_cao_bang,
    content_ha_long_to_ninh_binh_transport,
    content_dong_hoi_to_hue_transport,
    content_da_lat_to_mui_ne_transport,
    content_where_to_stay_in_vinh_hy,
    content_quy_nhon_to_nha_trang_transport,
]
...
def main():
    print(f"=== Deploying {len(POSTS)} Stage 84 Pillars to VPS (269 -> 276 pages) ===")
    ...
    sftp = ssh.open_sftp()
    with sftp.file('/tmp/stage84_posts.json', 'w') as f:
...
$posts = json_decode(file_get_contents('/tmp/stage84_posts.json'), true);
$inserted = [];
    ...
    if ($post_id && !is_wp_error($post_id)) {
        ...
        $wpdb->update(
            ...
            [
                ...
                'post_modified_gmt' => '2026-09-26 10:45:00'
            ],
            ['ID' => $post_id]
        );
        $inserted[] = [
...
file_put_contents('/tmp/stage84_deployed.json', json_encode($inserted));
"""
    with sftp.file('/tmp/deploy_stage84.php', 'w') as f:
        f.write(php_script)
    sftp.close()

    print("Executing PHP insertion via WP-CLI...")
    cmd = f"wp eval-file /tmp/deploy_stage84.php --path={WP_PATH} --allow-root"
    ...
    sftp = ssh.open_sftp()
    with sftp.file('/tmp/stage84_deployed.json', 'r') as f:
        deployed_data = json.load(f)
    sftp.close()

    ssh.exec_command("rm -f /tmp/deploy_stage84.php /tmp/stage84_posts.json /tmp/stage84_deployed.json")
    ...
    if all_ok:
        print("\nALL 7 STAGE 84 PILLARS LIVE & VERIFIED ON PRODUCTION VPS!")
    else:
