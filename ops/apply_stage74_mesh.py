# -*- coding: utf-8 -*-
"""
VietnamGuide Stage 74: Apply Internal Link Mesh to Production VPS
"""
...
def main():
    ...
    ops_file = os.path.join(ops_dir, 'stage74_mesh_ops.json')
    ...
    print(f"=== Applying Stage 74 Mesh: {len(mesh_dict)} posts to update ===")
    ...
    sftp = ssh.open_sftp()
    with sftp.file('/tmp/stage74_mesh_apply.json', 'w') as f:
...
$mesh = json_decode(file_get_contents('/tmp/stage74_mesh_apply.json'), true);
$count = 0;
    ...
    if (is_wp_error($result)) {
        ...
        $wpdb->update(
            ...
            [
                ...
                'post_modified_gmt' => '2026-09-23 15:00:00'
            ],
            ['ID' => $post_id]
        );
...
"""
    with sftp.file('/tmp/apply_stage74_mesh.php', 'w') as f:
        f.write(php_script)
    sftp.close()

    cmd = f"wp eval-file /tmp/apply_stage74_mesh.php --path={WP_PATH} --allow-root"
    ...
    ssh.exec_command("rm -f /tmp/apply_stage74_mesh.php /tmp/stage74_mesh_apply.json")
    print("Purging LiteSpeed cache...")
    ssh.exec_command(f"wp litespeed-purge all --path={WP_PATH} --allow-root")
    ssh.close()
    print("Stage 74 Mesh applied and cache purged.")
