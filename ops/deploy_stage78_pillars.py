# -*- coding: utf-8 -*-
"""
VietnamGuide Stage 78: Deploy 7 High-Intent Pillars to Production VPS
Expands coverage from 227 to 234 pages.
"""
...
def main():
    print(f"=== Deploying {len(POSTS)} Stage 78 Pillars to VPS (227 -> 234 pages) ===")
    ...
    sftp = ssh.open_sftp()
    with sftp.file('/tmp/stage78_posts.json', 'w') as f:
...
$posts = json_decode(file_get_contents('/tmp/stage78_posts.json'), true);
$inserted = [];
    ...
    if ($post_id && !is_wp_error($post_id)) {
        ...
        $wpdb->update(
            ...
            [
                ...
                'post_modified_gmt' => '2026-09-24 15:30:00'
            ],
            ['ID' => $post_id]
        );
        $inserted[] = [
...
file_put_contents('/tmp/stage78_deployed.json', json_encode($inserted));
"""
    with sftp.file('/tmp/deploy_stage78.php', 'w') as f:
        f.write(php_script)
    sftp.close()

    print("Executing PHP insertion via WP-CLI...")
    cmd = f"wp eval-file /tmp/deploy_stage78.php --path={WP_PATH} --allow-root"
    ...
    sftp = ssh.open_sftp()
    with sftp.file('/tmp/stage78_deployed.json', 'r') as f:
        deployed_data = json.load(f)
    sftp.close()

    ssh.exec_command("rm -f /tmp/deploy_stage78.php /tmp/stage78_posts.json /tmp/stage78_deployed.json")
    ...
    if all_ok:
        print("\nALL 7 STAGE 78 PILLARS LIVE & VERIFIED ON PRODUCTION VPS!")
    else:
