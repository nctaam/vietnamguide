<?php
/**
 * Publish the 21 Days in Vietnam guide using the EEAT template direction.
 *
 * Run from the WordPress root with:
 * wp eval-file ops/apply-21-days-itinerary-guide.php --allow-root
 */
...
{
    $value = getenv('VG_FORCE_21_DAY_ITINERARY_REPUBLISH');
...
function vg_ops_publish_author_id(?WP_Post $existing_page = null): int
{
    ...
    vg_ops_fail('Could not resolve a valid WordPress author for the 21 Days itinerary guide.');
}
...
function vg_ops_refresh_itineraries_hub(): void
{
    ...
    $marker = '<!-- vg-itinerary-21-day-hub-note:v1 -->';
    $block = <<<'HTML'
<!-- wp:group {"className":"vg-hub-note"} -->
<div class="wp-block-group vg-hub-note"><h3>Use 21 days for a full-country route with margin</h3><p>The <a href="/itineraries/21-days-in-vietnam/">21 Days in Vietnam guide</a> helps first-time visitors decide whether three weeks should become a full-country route with one deliberate extension, or a slower north-central-south route with better weather and transfer buffers.</p></div>
<!-- /wp:group -->
HTML;
    ...
    if (str_contains($hub->post_content, $marked_block)) {
        vg_ops_log('Skipped Itineraries hub refresh: current 21-day note already present.');
        return;
    }
    ...
    if (str_contains($hub->post_content, $marker)) {
        ...
        if (! is_string($updated_content) || $replacement_count !== 1) {
            vg_ops_fail('Could not confidently refresh the Itineraries hub 21-day note.');
        }
    } else {
    ...
    if (is_wp_error($result)) {
        vg_ops_fail('Could not refresh Itineraries hub 21-day note: ' . $result->get_error_message());
    }

    if ((int) $result <= 0) {
        vg_ops_fail('Could not refresh Itineraries hub 21-day note: WordPress returned an empty post ID.');
    }

    vg_ops_log("Refreshed Itineraries hub 21-day note: {$hub->ID}");
}
...
function vg_ops_add_related_route_to_page(string $path, string $label, string $route_line): void
{
    ...
    if (! $changed_route) {
        vg_ops_log("Skipped related-route refresh for {$label}: 21 Days line is already current.");
        return;
    }

    update_post_meta($page->ID, 'vg_eeat_related_routes', implode("\n", $next_lines));
    vg_ops_log("Upserted 21 Days related route to {$label}: {$page->ID}");
}

function vg_ops_repair_side_effects_enabled(): bool
{
    $value = getenv('VG_REPAIR_21_DAY_ITINERARY_LINKS');
...
{
    $route_line = '21 Days in Vietnam | /itineraries/21-days-in-vietnam/ | Decide whether three weeks should become a full-country route with one deliberate extension or a slower north-central-south route with more buffer.';
...
$page = get_page_by_path('itineraries/21-days-in-vietnam', OBJECT, 'page');
...
if ($page instanceof WP_Post && $page->post_status === 'publish' && vg_ops_repair_side_effects_enabled()) {
    vg_ops_log('Repairing 21 Days guide hub and inbound related-route side effects without rewriting the guide.');
    vg_ops_refresh_itineraries_hub();
    vg_ops_refresh_inbound_related_routes();
    vg_ops_log("Repaired 21 Days guide side effects: {$page->ID} " . get_permalink($page));
    return;
}

if ($page instanceof WP_Post && $page->post_status !== 'draft' && ! $allow_republish) {
    vg_ops_fail('21 Days itinerary guide is not a draft. Set VG_FORCE_21_DAY_ITINERARY_REPUBLISH=1 to overwrite existing published content.');
}

if ($page instanceof WP_Post) {
    $permalink = get_permalink($page);
    vg_ops_log("Preflight 21 Days itinerary guide: {$page->ID} {$page->post_status} {$permalink}");
} else {
    vg_ops_log('Preflight 21 Days itinerary guide: no existing page found; creating a child page under /itineraries/.');
}
...
<div class="wp-block-cover alignfull is-dark vg-guide-hero-cover" style="min-height:620px"><span aria-hidden="true" class="wp-block-cover__background has-ink-background-color has-background-dim"></span><img class="wp-block-cover__image-background" alt="Limestone islands in Ha Long Bay, Vietnam, used for a 21-day Vietnam itinerary guide" src="{$guide_hero_image}" data-object-fit="cover" loading="eager" decoding="async" fetchpriority="high"><div class="wp-block-cover__inner-container">
...
<h1 class="wp-block-heading vg-display vg-guide-title">21 Days in Vietnam</h1>
<!-- /wp:heading -->
<!-- wp:paragraph {"className":"vg-guide-lede"} -->
<p class="vg-guide-lede">Three weeks is enough for a graceful Vietnam route if you protect the transfer days. The strongest first-trip plan is not the ten-day route stretched with every famous stop; it is a full-country route with margin, fewer rushed mornings, better weather choices, and one carefully chosen extension.</p>
<!-- /wp:paragraph -->
<!-- wp:paragraph {"className":"vg-field-note"} -->
<p class="vg-field-note">Field note: spend the extra week on margin and contrast before you spend it on another airport.</p>
<!-- /wp:paragraph -->
</div>
<!-- /wp:group -->
<!-- wp:html -->
...
<!-- wp:group {"className":"vg-concierge-verdict vg-itinerary-21day-concierge-verdict","layout":{"type":"constrained"}} -->
<div class="wp-block-group vg-concierge-verdict vg-itinerary-21day-concierge-verdict">
<!-- wp:paragraph {"className":"vg-kicker"} -->
<p class="vg-kicker">Concierge verdict</p>
<!-- /wp:paragraph -->
<!-- wp:paragraph {"className":"vg-verdict-lede"} -->
<p class="vg-verdict-lede"><strong>Most first-time visitors should use 21 days for Hanoi, Ninh Binh, a bay overnight, Hue, Hoi An, Ho Chi Minh City, and one southern or central extension.</strong> Choose only one extra module: Mekong, Phong Nha, beach time, mountains, or a slower central stay. The mistake is treating three weeks as permission to add every famous place.</p>
<!-- /wp:paragraph -->
...
<!-- wp:group {"className":"vg-at-a-glance vg-itinerary-21day-at-a-glance","layout":{"type":"constrained"}} -->
<div class="wp-block-group vg-at-a-glance vg-itinerary-21day-at-a-glance">
<!-- wp:paragraph {"className":"vg-kicker"} -->
<p class="vg-kicker">Route brief</p>
<!-- /wp:paragraph -->
<!-- wp:heading -->
<h2 class="wp-block-heading">21-day Vietnam itinerary at a glance</h2>
<!-- /wp:heading -->
<!-- wp:html -->
<table class="vg-decision-table">
<tbody>
<tr><td data-label="Decision"><strong>Best default route</strong></td><td data-label="VietnamGuide recommendation">Hanoi, Ninh Binh, Ha Long or Lan Ha Bay, Hue, Hoi An or Da Nang, Ho Chi Minh City, and one deliberate extension only if it replaces something else.</td></tr>
<tr><td data-label="Decision"><strong>Airport shape</strong></td><td data-label="VietnamGuide recommendation">Arrive in Hanoi and depart from Ho Chi Minh City or Da Nang if the flight pricing and time savings are real.</td></tr>
<tr><td data-label="Decision"><strong>Extension rule</strong></td><td data-label="VietnamGuide recommendation">Use the extra week for one meaningful extension: Mekong, Phong Nha, mountains, beach rest, or slower central Vietnam. Not all of them.</td></tr>
<tr><td data-label="Decision"><strong>Cut first</strong></td><td data-label="VietnamGuide recommendation">Remove duplicate one-night add-ons, a second scenery module, or a weak final flight day before you start cutting the route's core.</td></tr>
<tr><td data-label="Decision"><strong>Do not use this route if</strong></td><td data-label="VietnamGuide recommendation">You only want one region, one beach base, or a trip with almost no domestic transfers.</td></tr>
</tbody>
</table>
<!-- /wp:html -->
</div>
...
<figure class="vg-guide-photo"><img src="{$son_river_image}" alt="Son River landscape in Quang Binh province near Phong Nha, Vietnam" loading="lazy" decoding="async"><figcaption>Phong Nha can be excellent, but it needs to replace something, not hide inside an already full route. Image: <a href="{$son_river_credit_url}" target="_blank" rel="license noopener">BacLuong / CC BY-SA 4.0</a>.</figcaption></figure>
</div>
<!-- /wp:html -->

<!-- vg-itinerary-21day-planning-flow:v1 -->
<!-- wp:group {"className":"vg-travel-guide-flow vg-itinerary-21day-planning-flow","layout":{"type":"constrained"}} -->
<div class="wp-block-group vg-travel-guide-flow vg-itinerary-21day-planning-flow">
<!-- wp:paragraph {"className":"vg-kicker"} -->
<p class="vg-kicker">21-day planning flow</p>
<!-- /wp:paragraph -->
<!-- wp:heading -->
<h2 class="wp-block-heading">The six decisions that keep three weeks from becoming a blur</h2>
<!-- /wp:heading -->
<!-- wp:html -->
<ol class="vg-travel-flow-list" aria-label="21-day Vietnam route planning order">
<li><span>01</span><strong>Flight shape</strong><p>Check open-jaw arrival and departure before accepting a backtrack across the country.</p></li>
<li><span>02</span><strong>Route spine</strong><p>Choose the north-central-south spine first: Hanoi, countryside, bay, central heritage/food, Ho Chi Minh City.</p></li>
<li><span>03</span><strong>Extension</strong><p>Select one deliberate extension: Mekong, Phong Nha, mountain scenery, beach rest, or a slower central stay.</p></li>
<li><span>04</span><strong>Hotel changes</strong><p>Count every packing morning. If the route has too many one-night stops, it is not premium yet.</p></li>
<li><span>05</span><strong>Season pressure</strong><p>Use regional weather to decide where the flexible days belong, especially around central-coast rain windows.</p></li>
<li><span>06</span><strong>Final buffer</strong><p>Protect the last 36 hours near the departure city before adding any remote finale.</p></li>
</ol>
<!-- /wp:html -->
</div>
<!-- /wp:group -->
...
<!-- wp:heading -->
<h2 class="wp-block-heading">Choose the right 21-day route family</h2>
<!-- /wp:heading -->
<!-- wp:html -->
<div class="vg-travel-route-family vg-itinerary-21day-route-family" aria-label="21-day Vietnam route family comparison">
<article><p class="vg-route-family-label">Best default</p><h3>Full-country with margin</h3><p>Hanoi, Ninh Binh, bay, Hue, Hoi An/Da Nang, Ho Chi Minh City, and one deliberate extension.</p><strong>Most complete</strong></article>
<article><p class="vg-route-family-label">Most relaxed</p><h3>North plus central slow route</h3><p>Drop the south when comfort, food, landscapes, and fewer airports matter more than national coverage.</p><strong>Lowest friction</strong></article>
<article><p class="vg-route-family-label">Scenery first</p><h3>North-heavy route</h3><p>Use the trip for Hanoi, Ninh Binh, bay, and one mountain area, then cut the full-country promise honestly.</p><strong>Best landscapes</strong></article>
<article><p class="vg-route-family-label">Warm-weather pivot</p><h3>Central and south route</h3><p>Lead with Da Nang, Hoi An, Hue, Ho Chi Minh City, Mekong, and one beach or city-rest extension.</p><strong>Best weather pivot</strong></article>
</div>
<!-- /wp:html -->
...
<!-- /wp:heading -->
<!-- wp:paragraph -->
<p>The easiest way to make a three-week itinerary feel expensive is to stop changing hotels without a reason. Use nights as the real planning currency: they reveal whether the trip is a route or a blur.</p>
<!-- /wp:paragraph -->
<!-- wp:html -->
<table class="vg-decision-table vg-itinerary-21day-night-allocation">
<thead>
...
<!-- wp:heading -->
<h2 class="wp-block-heading">What the extra week actually buys</h2>
<!-- /wp:heading -->
<!-- wp:paragraph -->
<p>The difference between a good 10-day route and a good 21-day route is not seven more pins on the map. The real value is a better start, a stronger middle, and one extension that has enough room to mean something.</p>
<!-- /wp:paragraph -->
<!-- wp:html -->
<table class="vg-decision-table vg-itinerary-21day-extra-days-value">
<thead>
...
<!-- wp:heading -->
<h2 class="wp-block-heading">A practical 21-day Vietnam itinerary</h2>
<!-- /wp:heading -->
<!-- wp:paragraph -->
...
<!-- /wp:paragraph -->
<!-- wp:html -->
<div class="vg-timeline vg-itinerary-21day-day-plan">
...
<div class="vg-timeline-item"><div class="vg-day">Day 2</div><div><h3>Hanoi orientation</h3><p>Use a food walk, museum, lake district, or old-quarter route to understand the city before leaving it.</p></div></div>
<div class="vg-timeline-item"><div class="vg-day">Day 3</div><div><h3>Ninh Binh overnight</h3><p>Go overnight if you want calmer mornings. Make it a day trip only if hotel changes need to stay minimal.</p></div></div>
<div class="vg-timeline-item"><div class="vg-day">Day 4</div><div><h3>Ninh Binh depth and bay positioning</h3><p>Use one main landscape experience, then position for the bay transfer rather than stacking every viewpoint.</p></div></div>
<div class="vg-timeline-item"><div class="vg-day">Day 5</div><div><h3>Ha Long or Lan Ha Bay overnight</h3><p>Book the cruise as a logistics product: pickup point, port, cabin, cancellation policy, and weather process matter.</p></div></div>
<div class="vg-timeline-item"><div class="vg-day">Day 6</div><div><h3>Bay return and Hue transfer</h3><p>Protect this day. The cruise return, road transfer, flight, and hotel check-in can swallow time fast.</p></div></div>
<div class="vg-timeline-item"><div class="vg-day">Day 7</div><div><h3>Hue</h3><p>Use Hue for imperial history, slower food, and context. If heritage is not your priority, convert this into a central buffer night.</p></div></div>
...
<div class="vg-timeline-item"><div class="vg-day">Day 10</div><div><h3>Hoi An or Da Nang buffer</h3><p>Use the middle of the trip to absorb weather, slow down, or move one activity without breaking the route.</p></div></div>
<div class="vg-timeline-item"><div class="vg-day">Day 11</div><div><h3>Central extension or recovery</h3><p>Add Phong Nha, a beach rest, or a second Hoi An/Da Nang night only if it replaces something weaker elsewhere.</p></div></div>
<div class="vg-timeline-item"><div class="vg-day">Day 12</div><div><h3>Fly to Ho Chi Minh City</h3><p>Keep the first southern day urban and flexible. Late flights and traffic can shrink the useful part of the day.</p></div></div>
<div class="vg-timeline-item"><div class="vg-day">Day 13</div><div><h3>Ho Chi Minh City</h3><p>Choose city history, food, markets, architecture, or a guided local lens. Do not treat the city as only a transit stop.</p></div></div>
<div class="vg-timeline-item"><div class="vg-day">Day 14</div><div><h3>Mekong Delta</h3><p>Use one southern river day for contrast, food, and slower movement. Do not squeeze it onto departure day.</p></div></div>
<div class="vg-timeline-item"><div class="vg-day">Day 15</div><div><h3>Ho Chi Minh City reset</h3><p>Return to the city or keep a recovery day before the trip's deliberate extension. This is where many itineraries forget to breathe.</p></div></div>
<div class="vg-timeline-item"><div class="vg-day">Day 16</div><div><h3>Fly to Da Lat or a chosen southern extension</h3><p>Select the extension only if it gives a new kind of memory: hill climate, coffee, gardens, beach, or cave time.</p></div></div>
<div class="vg-timeline-item"><div class="vg-day">Day 17</div><div><h3>Extension depth</h3><p>Keep the extension to one clear chapter. The point is contrast, not collecting another city for its own sake.</p></div></div>
<div class="vg-timeline-item"><div class="vg-day">Day 18</div><div><h3>Extension slow day</h3><p>Use the second extension day for slower movement, better meals, and an easier flight or road return later.</p></div></div>
<div class="vg-timeline-item"><div class="vg-day">Day 19</div><div><h3>Return to Ho Chi Minh City</h3><p>Come back early enough to protect the final city night. The route earns its premium feel here, not only in the famous stops.</p></div></div>
<div class="vg-timeline-item"><div class="vg-day">Day 20</div><div><h3>Final Ho Chi Minh City day</h3><p>Leave this open for a museum, food walk, shopping, or one last guided context experience.</p></div></div>
<div class="vg-timeline-item"><div class="vg-day">Day 21</div><div><h3>Depart from Ho Chi Minh City</h3><p>Keep departure day conservative. If the flight is late, use a close city experience rather than a distant excursion.</p></div></div>
</div>
<!-- /wp:html -->
...
<!-- /wp:heading -->
<!-- wp:paragraph -->
<p>A three-week itinerary fails most often at the handoffs, not at the headline destinations. These are the days to plan like an operator, not like a brochure.</p>
<!-- /wp:paragraph -->
<!-- wp:html -->
<table class="vg-decision-table vg-itinerary-21day-transfer-pressure">
<thead>
...
<!-- wp:heading -->
<h2 class="wp-block-heading">Four route variants that still fit 21 days</h2>
<!-- /wp:heading -->
<!-- wp:paragraph -->
...
<!-- /wp:paragraph -->
<!-- wp:html -->
<table class="vg-decision-table vg-itinerary-21day-route-variants">
<thead>
...
<!-- wp:heading -->
<h2 class="wp-block-heading">Where the extra week should go</h2>
<!-- /wp:heading -->
<!-- wp:paragraph -->
<p>The extra days in a three-week trip should create contrast or resilience. They should not create a second trip hidden inside the first one.</p>
<!-- /wp:paragraph -->
<!-- wp:html -->
<table class="vg-decision-table vg-itinerary-21day-extension-matrix">
<thead>
...
<!-- /wp:heading -->
<!-- wp:paragraph -->
<p>Three weeks gives you enough room to spend deliberately. The premium move is not upgrading everything. It is putting money where the route becomes easier, safer, or more memorable.</p>
<!-- /wp:paragraph -->
<!-- wp:html -->
<table class="vg-decision-table vg-itinerary-21day-base-strategy">
<thead>
...
<!-- wp:heading -->
<h2 class="wp-block-heading">Season pivots for a three-week route</h2>
<!-- /wp:heading -->
<!-- wp:paragraph -->
<p>Vietnam does not have one simple best month. A three-week route usually crosses weather zones, so the better question is which part of the country deserves your flexibility.</p>
<!-- /wp:paragraph -->
<!-- wp:html -->
<table class="vg-decision-table vg-itinerary-21day-season-pivots">
<thead>
...
<!-- wp:heading -->
<h2 class="wp-block-heading">Slowdown rules that make three weeks feel premium</h2>
<!-- /wp:heading -->
<!-- wp:paragraph -->
...
<!-- /wp:paragraph -->
<!-- wp:html -->
<table class="vg-decision-table vg-itinerary-swap-skip vg-itinerary-21day-swap-skip">
<thead>
...
<!-- /wp:heading -->
<!-- wp:paragraph -->
<p>The same 21 days should not be sold to every traveler. Use these adaptations before choosing hotels, tours, or domestic flights.</p>
<!-- /wp:paragraph -->
<!-- wp:html -->
<table class="vg-decision-table vg-itinerary-21day-audience-adaptations">
<thead>
...
<!-- wp:heading -->
<h2 class="wp-block-heading">Booking sequence for 21 days</h2>
<!-- /wp:heading -->
<!-- wp:paragraph -->
<p>Do not book this route from day one forward. Book the constraints first, then let the softer days fit around them.</p>
<!-- /wp:paragraph -->
<!-- wp:html -->
<table class="vg-decision-table vg-itinerary-21day-prebook-flex">
<thead>
...
<!-- wp:list {"className":"vg-check-list vg-itinerary-21day-booking-sequence"} -->
<ul class="wp-block-list vg-check-list vg-itinerary-21day-booking-sequence">
...
<!-- /wp:heading -->
<!-- wp:paragraph -->
<p>Use this audit when your spreadsheet looks finished. A route can be technically possible and still be a poor three-week trip if it hides too many weak mornings, late transfers, and fragile assumptions.</p>
<!-- /wp:paragraph -->
<!-- wp:html -->
<table class="vg-decision-table vg-itinerary-21day-planning-audit">
<thead>
...
<!-- wp:heading -->
<h2 class="wp-block-heading">Short answers for common three-week mistakes</h2>
<!-- /wp:heading -->
<!-- wp:paragraph -->
<p>These are the questions that usually reveal whether a 21-day plan is designed for the traveler or copied from a search result.</p>
<!-- /wp:paragraph -->
<!-- wp:html -->
<div class="vg-faq-list vg-travel-guide-mistakes vg-itinerary-21day-mistakes">
<details><summary>Should 21 days include Sapa or Ha Giang?</summary><p>Only if northern landscapes are the point of the trip. Do not add Sapa or Ha Giang to the balanced north-central-south route unless you remove the south, reduce central Vietnam, or accept a much faster trip.</p></details>
...
<!-- wp:heading -->
<h2 class="wp-block-heading">When 14 days is actually better</h2>
<!-- /wp:heading -->
<!-- wp:paragraph -->
<p>More days do not automatically make the trip better. If the extra week forces awkward flights, poor weather, a remote detour you do not truly want, or a higher budget that does not buy comfort, use the <a href="/itineraries/14-days-in-vietnam/">14 Days in Vietnam guide</a> and keep the route sharper.</p>
<!-- /wp:paragraph -->
<!-- wp:paragraph -->
<p>A strong 14-day trip plus a future return can be wiser than a 21-day trip that spends its advantage on movement. This is especially true for families, older travelers, honeymooners, and anyone visiting in a season where one region clearly has better conditions than the others.</p>
<!-- /wp:paragraph -->
...
$required_content_markers = [
    'hero marker' => 'vg-itinerary-21day-hero:v1',
    'concierge verdict' => 'vg-itinerary-21day-concierge-verdict',
    'editorial proof panel' => 'vietnamguide/editorial-proof-panel',
    'at a glance' => 'vg-itinerary-21day-at-a-glance:v1',
    'photo grid' => 'vg-itinerary-21day-photo-grid:v1',
    'planning flow' => 'vg-itinerary-21day-planning-flow:v1',
    'route family' => 'vg-itinerary-21day-route-family:v1',
    'transfer pressure' => 'vg-itinerary-21day-transfer-pressure:v1',
    'day plan' => 'vg-itinerary-21day-day-plan:v1',
    'extension matrix' => 'vg-itinerary-21day-extension-matrix:v1',
    'season pivots' => 'vg-itinerary-21day-season-pivots:v1',
    'swap skip' => 'vg-itinerary-21day-swap-skip:v1',
    'prebook flex' => 'vg-itinerary-21day-prebook-flex:v1',
    'mistakes' => 'vg-itinerary-21day-mistakes:v1',
    'booking sequence' => 'vg-itinerary-21day-booking-sequence:v1',
    'planning audit' => 'vg-itinerary-21day-planning-audit:v1',
...
vg_ops_assert_internal_page_links_are_published('21 Days in Vietnam guide', $content);

$post_args = [
    'post_type'      => 'page',
    'post_title'     => '21 Days in Vietnam',
    'post_name'      => '21-days-in-vietnam',
    'post_parent'    => (int) $parent->ID,
    'post_status'    => 'publish',
    'post_author'    => vg_ops_publish_author_id($page instanceof WP_Post ? $page : null),
    'post_content'   => $content,
    'post_excerpt'   => 'Evidence-led 21-day Vietnam itinerary for international visitors, with route-shape decisions, night allocation, transfer pressure checks, route variants, licensed photography, and booking sequence.',
...
if (is_wp_error($result)) {
    vg_ops_fail('Could not publish 21 Days itinerary guide: ' . $result->get_error_message());
}

if ((int) $result <= 0) {
    vg_ops_fail('Could not publish 21 Days itinerary guide: WordPress returned an empty post ID.');
}

$page_id = (int) $result;

update_post_meta($page_id, 'rank_math_title', '21 Days in Vietnam: Best Three-Week Route for First Trips');
update_post_meta($page_id, 'rank_math_description', 'Evidence-led 21-day Vietnam itinerary for international travelers, with a full-country route, one deliberate extension, night allocation, transfer checks, photos, and booking sequence.');
update_post_meta($page_id, 'rank_math_focus_keyword', '21 days in Vietnam');
update_post_meta($page_id, 'vg_eeat_primary_decision', 'Choose the strongest 21-day Vietnam route shape before booking flights, hotels, cruises, and domestic transfers.');
...
vg_ops_assert_related_route_meta_links_are_published('21 Days in Vietnam related routes', $related_routes);
...
if (get_post_status($page_id) !== 'publish') {
    vg_ops_fail('21 Days itinerary guide was updated but is not published.');
}
...
vg_ops_log("Published 21 Days in Vietnam itinerary guide: {$page_id} {$updated_permalink}");
